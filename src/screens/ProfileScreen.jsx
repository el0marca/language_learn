...
import { Text, View, StyleSheet, TextInput, ImageBackground, Dimensions, Image, ActivityIndicator, Keyboard, TouchableOpacity } from 'react-native'
...
GoogleSignin.configure({
  webClientId: '1078824412746-5c3l9ke687snskfjpk0gucn1mhikj9fp.apps.googleusercontent.com',
});
...
export const ProfileScreen = () => {
  ...
  const bottomTab=useSelector(state=>state.bottomTab.show)
  ...
  useEffect(() => { if (user) getData(user.uid) }, [user])
  ...
  return (
    <ImageBackground source={require('../img/bg/tasksBg.jpg')} style={{ flex: 1 }}>
      ...
      <View style={{ flex: 8, backgroundColor: '#fff', borderTopLeftRadius: 40, borderTopRightRadius: 40, alignItems: 'center' }}>
          ...
          <>
            ...
            {!loginMode ?
              <View style={{ alignItems: 'center' }}>
                <ButtonItem fnc={signUpWithEmail} descr={'Qeydiyyatdan keçmək'} img={require('../img/mail.png')}/>
                <ButtonItem fnc={onGoogleButtonPress} descr={'Google ilə daxil ol'} img={require('../img/google.png')}/>
                <TouchableOpacity style={{paddingTop:20}} onPress={() => { setLoginMode(true), setErrorInfo(''), onChangePassword('') }}>
                  <Text style={{fontFamily: 'SFUIDisplay-Bold', fontSize:16}}>Həsabınız var? Daxil olun</Text>
                </TouchableOpacity>
              </View>
              : null}
            {loginMode ?
              <View style={{ alignItems: 'center' }}>
                {!forgotPassword && 
              <ButtonItem fnc={signInWithEmail} descr={'Daxil olmaq'} img={require('../img/mail.png')}/>}
                {forgotPassword && 
              <ButtonItem fnc={passwordReset} descr={'Şifrəni sıfırla'} img={require('../img/resetEmail.png')}/>}
                <View style={{ flexDirection: 'row', justifyContent: 'space-around' }}>
                  {!forgotPassword && <TouchableOpacity activeOpacity={0.9} onPress={() => { !forgotPassword && (setForgotPassword(true), setErrorInfo('')), forgotPassword && passwordReset() }}>
                    <Text style={{ fontFamily: 'SFUIDisplay-Bold', paddingTop: 10, textAlign: 'center', fontSize: 16 }}>Şifrəni unutmusan?</Text>
                  </TouchableOpacity>}
                </View>
              </View> : null}
          </>
          ...
          <View style={{ width: '90%', alignItems: 'center', flex: 1 }}>
            <View style={{ flex: 4 }}>
              <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center', }}>
                <Text style={{ paddingRight: 10 }}>{user.displayName || user.email}</Text>
...
const ButtonItem = ({ fnc, descr, img }) => {
  return (
    <TouchableOpacity activeOpacity={.8} style={{ backgroundColor: '#fff', padding: 10, textAlign: 'center', borderRadius: 5, fontSize: 19, marginBottom: 5, paddingHorizontal: 20, minWidth: '80%', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', width: '80%', elevation: 5 }} onPress={fnc}>
    <View style={{ flex: 1 }}>
      <Image style={{ width: 30, height: 30 }} source={img} />
    </View>
    <Text style={{ color: '#555', flex: 9, fontSize: 17, fontFamily: 'SFUIDisplay-Bold', textAlign: 'center' }}>{descr}</Text>
  </TouchableOpacity>
  )
}
