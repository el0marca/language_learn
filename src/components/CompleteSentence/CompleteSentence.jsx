        ...
        <ImageBackground source={require('../../img/bg/tasksBg.jpg')} style={{ flex: 1, resizeMode: "center", justifyContent: "center" }}>
            ...
            <View style={s.wrapper}>
                ...
                <Animated.View style={[s.choiceWrapper, { opacity: outputAnim }]}>
                    <View style={{ flex: 1 }}>
                        {answered && <ResultModal result={result} sentence={transSentence} transSentence={sentences.sntc.replace('___', `${sentences.ra.trim()}`)} errorData={errorData} openReportWindow={()=>setReportMode(true)} />}
                    </View>
                    <View style={{ width: '100%', justifyContent: 'center', flex: 1, paddingHorizontal: 20 }}>
                        <TouchableOpacity disabled={!value} onPress={!answered ? () => { play(), check() } : answered && !isReady ? next : answered && isReady ? () => navigation.goBack() : next}>
