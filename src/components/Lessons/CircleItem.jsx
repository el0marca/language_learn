...
export const CircleItem = ({ progress, index, subject }) => {
    ...
    // const [ere, rere] = React.useState()
    // (async function () { rere(await storage().ref('/img/circles/1.png').getDownloadURL())}())
    // (async function (){
    //     rere(await storage().ref(`/img/circles/${index}.png`).getDownloadURL())
    //     console.log('re')
    // })()
    // console.log(index)
    // console.log(ere)
    // const [ere, rere] = React.useState()
    // async function fd () {rere(await storage().ref('/img/background/lessonBg.png').getDownloadURL())}
    // fd()
    // console.log(ere)
    return (
