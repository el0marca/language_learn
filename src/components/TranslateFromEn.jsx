...
export const TranslateFromEn = ({ route }) => {
    ...
    const sentences = useSelector(state => state.enAzSentences.sentences[level][route.params.lessonIndex][num])
    const [wordNumber, setWordNumber] = useState(0)
    const [output, setOutput] = useState('')
    const [sentence, setSentence] = useState(sentences.sntc)
    const [transSentence, setTransSentence] = useState(sentences.ch)
    const [originSentence, setOriginSentence] = useState([...sentences.tr.split(' ')])
    ...
    useEffect(() => setSentence(sentences.sntc), [num])
    useEffect(() => setTransSentence(sentences.ch), [num])
    useEffect(() => setChoice(shuffle(transSentence.split(' '))), [sentence])
    useEffect(() => setOutput(''), [num])
    useEffect(() => setOriginSentence([...sentences.tr.split(' ')]), [transSentence])
    useEffect(() => setWordNumber(0), [transSentence])
    useEffect(() => setResult(sentences.tr === output), [output])
    useEffect(() => { if (num === 9 && result) { setReady(true) } })

    const progressValue = route.params.lessonIndex * 6 + 6;
    ...
    return (
        ...
        <View style={style.wrapper}>
                ...
                <Animated.View style={{ marginTop: 20, justifyContent: 'center', flex: 2, opacity: fadeAnim }}>
                    <CommonButton result={result} isReady={isReady} next={next} num={route.params.num} />
                </Animated.View>
            </View>
        </ImageBackground>   
    )
